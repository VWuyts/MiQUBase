 ...
 */
		...
		&& (strcmp($_SESSION['role'], 'administrator') == 0 || strcmp($_SESSION['role'], 'executor') == 0 || strcmp($_SESSION['role'], 'readonly') == 0 || strcmp($_SESSION['role'], 'creator') == 0))
	{
		...
		// Set variables
		$reportName = 'MiQUBase report: samples to repeat';
		$headerRow = 8;
		...
		$objPHPExcel->setActiveSheetIndex(0)
					->setCellValue('A'.$headerRow, 'Sample name')
					->setCellValue('B'.$headerRow, 'Sample reception date')
					->setCellValue('C'.$headerRow, 'Run number')
					->setCellValue('D'.$headerRow, 'Run start date')
					->setCellValue('E'.$headerRow, 'Division');
		$objPHPExcel->getActiveSheet()->getStyle('A'.$headerRow .':E'.$headerRow)->getFont()->setBold(true);
		...
		{
			$objPHPExcel->setActiveSheetIndex(0)
						->setCellValue('A'.($i+$headerRow+1), $_SESSION['report'][$i]['samplename'])
						->setCellValue('B'.($i+$headerRow+1), getEuroDate($_SESSION['report'][$i]['receptiondate']))
						->setCellValue('C'.($i+$headerRow+1), $_SESSION['report'][$i]['runnumber'])
						->setCellValue('D'.($i+$headerRow+1), getEuroDate($_SESSION['report'][$i]['startdate']))
						->setCellValue('E'.($i+$headerRow+1), $_SESSION['report'][$i]['divisionname']);
		}
		// Rename worksheet
		$objPHPExcel->getActiveSheet()->setTitle('Samples_to_repeat');
		if (!$_SESSION['excel'])
		{
		...
		// Create file name
		$filename = "Samples_to_repeat_". date("Ymd") ."_". $_SESSION['user'];
