 ...
 */
	...
	if (isset($_SESSION['role']) && (strcmp($_SESSION['role'], 'administrator') == 0 || strcmp($_SESSION['role'], 'creator') == 0))
	{
		...
		createHead(true, 'MiQUBase division', ['isActive'], null);
		...
		else
		{
			...
			$query = "SELECT DISTINCT * FROM division ORDER BY divisionname ASC;";
			...
			{
				echo("\t\t<p class='message'>There are currently no divisions loaded into MiQUBase.</p>\n");
				echo("\t\t<p><a href='administrativeTasks.php'>Back to tasks overview page</a></p>\n");
			}
			else
			{
				echo("\t\t<h1>Divisions loaded into MiQUBase</h1>\n");
				echo("\t\t<form method='post' action='statusDivisionChange.php'>\n");
				echo("\t\t\t<table>\n");
				...
				while($arr = pg_fetch_array($result))
				{
					...
					$divisionArr['status'][] = $currentStatus;
					$divisionArr['id'][] = $arr['divisionid'];
					echo("\t\t\t\t<tr>\n\t\t\t\t\t<td>".$arr['divisionname']."</td>\n\t\t\t\t\t<td>".$currentStatus."</td>\n");
					echo("\t\t\t\t\t<td><input class='checkbox' type='checkbox' name='".$arr['divisionid']."' value='change'></td>\n");
					echo("\t\t\t\t</tr>\n");
				}
				$_SESSION['input'] = $divisionArr;
				echo("\t\t\t</table>\n");
