 ...
 */
	...
	if (isset($_SESSION['role']) && isset($_SESSION['input']) && (strcmp($_SESSION['role'], 'administrator') == 0 || strcmp($_SESSION['role'], 'creator') == 0))
	{
		...
		{
			// Create array for updated divisionid
			$idArr = array();
			...
			for ($i = 0; $i < count($_SESSION['input']['id']); $i++)
			{
				...
				if (isset($_POST[$_SESSION['input']['id'][$i]]))
				{
					...
					else
					{
						...
						// Create log message
						$activitylogger->info('status change of division succeeded', ['user'=>$_SESSION['user'], 'divisionid'=>$_SESSION['input']['id'][$i], 'changedto'=>$params[0]]);
					}
				}
			}
			unset($_SESSION['input']);
			if (count($idArr) == 0)
			{
				echo("\t\t<p class='message'>No divisions have changed their status.</p>\n");
			}
			else
			{
				$query = "SELECT divisionname, isactive
							FROM division
							WHERE divisionid = ". $idArr[0];
				for ($i = 1; $i < count($idArr); $i++)
				{
					$query .= " OR divisionid = ".  $idArr[$i];
				}
				$query .= " ORDER BY divisionname ASC;";
				...
				{
					echo("\t\t<h1>Divisions of which the status is changed</h1>\n");
					echo("\t\t<table>\n");
