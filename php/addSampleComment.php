		...
		else
		{
			...
			if (isset($_POST['addRemark']))
			{
				if (empty($_POST['sample']))
				{
					...
					createMessagePage(["No samples of run ". $_SESSION['runnumber'], "were selected to add remarks in MiQUBase."],
				...
				else
				{
					...
					$samplesToComment = count($_POST['sample']);
					// Create update query
					$query = "UPDATE sample
								SET remark=$1
								WHERE sampleid=$2;";
					$result = false;
					$updatecounter = 0;
					for ($i = 0; $i < $samplesToComment; $i++)
					{
						$params = array();
						if (!empty($newRemarkArr[$_POST['sample'][$i]]))
						{
							...
							else
							{
								$updatecounter++;
								// Create log message
								$activitylogger->info('adding remark to sample in db succeeded', ['user'=>$_SESSION['user'], 'runnumber'=>$_SESSION['runnumber'] , 'sampleid'=>$_POST['sample'][$i], 'remark'=>$newRemarkArr[$_POST['sample'][$i]]]);
							}
						}
					}			
				}
				...
				pg_close($dbconn);
				// Redirect user to message page
				createMessagePage(["To ". $updatecounter ." samples of run ".$_SESSION['runnumber'], "remarks have been added in MiQUBase."],
			...
			if (isset($_POST['run']) && !empty($_POST['laneid']))
			{
				...
				$query = "SELECT sample.sampleid, sample.samplename, sample.remark,
							...
							ORDER BY project.projectnumber, sample.samplename;";
				...
				if (!$result)
				{
					...
					if (isset($_POST['addRemark'])) unset($_POST['addRemark']);
				...
				elseif (pg_num_rows($result) == 0)
				{
					...
					echo("\t\t<p class='message'>to which remarks can be added.</p>\n");
			...
			if (isset($_POST['run']) && !empty($_POST['laneid']))
			{
				...
				echo("\t\t<h1 class='top'>Check to which samples of run ". $_SESSION['runnumber'] ." remarks have to be added</h1>\n");
				echo("\t\t<form method='post' action=". htmlspecialchars($_SERVER['PHP_SELF']) .">\n");
				echo("\t\t\t<table>\n");
				for ($i = 0; $i < count($_SESSION['sampleidArr']); $i++)
				{
					...
					else
					{
						...
						if ($counter != 0)
						{
							...
							echo("\t\t\t\t<tr>\n\t\t\t\t\t<td colspan='3'>\n");
							echo("\t\t\t\t\t\t<input name='".$class."' class='checkbutton' type='button' value='Check all samples' />\n");
				...
				echo("\t\t\t\t<input class='button' type='submit' name='addRemark' value='Add to MiQUBase' />\n");
				echo("\t\t\t</div>\n\t\t</form>\n");
			}
		}
