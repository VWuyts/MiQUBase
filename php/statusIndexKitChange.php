 ...
 */
	...
	if (isset($_SESSION['role']) && isset($_SESSION['input']) && (strcmp($_SESSION['role'], 'administrator') == 0 || strcmp($_SESSION['role'], 'creator') == 0))
	{
		...
		createHead(true, 'MiQUBase indexKit', ['isActive'], null);
		...
		{
			// Create array for updated indexkitid
			$idArr = array();
			// Perform required updates
			$query = "UPDATE indexkit
						SET isactive = $1
						WHERE indexkitid = $2;";
			for ($i = 0; $i < count($_SESSION['input']['id']); $i++)
			{
				...
				if (isset($_POST[$_SESSION['input']['id'][$i]]))
				{
					...
					else
					{
						...
						// Create log message
						$activitylogger->info('status change of indexkit succeeded', ['user'=>$_SESSION['user'], 'indexkitid'=>$_SESSION['input']['id'][$i], 'changedto'=>$params[0]]);
					}
				}
			}
			unset($_SESSION['input']);
			if (count($idArr) == 0)
			{
				echo("\t\t<p class='message'>No indexKits have changed their status.</p>\n");
			}
			else
			{
				$query = "SELECT indexkitname, isactive
							FROM indexkit
							WHERE indexkitid = ". $idArr[0];
				for ($i = 1; $i < count($idArr); $i++)
				{
					$query .= " OR indexkitid = ".  $idArr[$i];
				}
				$query .= " ORDER BY indexkitname ASC;";
				...
				{
					echo("\t\t<h1>IndexKits of which the status is changed</h1>\n");
					echo("\t\t<table>\n");
