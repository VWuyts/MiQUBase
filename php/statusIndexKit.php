 ...
 */
	...
	if (isset($_SESSION['role']) && (strcmp($_SESSION['role'], 'administrator') == 0 || strcmp($_SESSION['role'], 'creator') == 0))
	{
		...
		else
		{
			...
			$query = "SELECT DISTINCT * FROM indexkit ORDER BY indexkitname ASC;";
			...
			{
				echo("\t\t<p class='message'>There are currently no indexKits loaded into MiQUBase.</p>\n");
				echo("\t\t<p><a href='administrativeTasks.php'>Back to tasks overview page</a></p>\n");
			}
			else
			{
				echo("\t\t<h1>IndexKits loaded into MiQUBase</h1>\n");
				echo("\t\t<form method='post' action='statusIndexKitChange.php'>\n");
				echo("\t\t\t<table>\n");
				...
				while($arr = pg_fetch_array($result))
				{
					...
					$indexkitArr['status'][] = $currentStatus;
					$indexkitArr['id'][] = $arr['indexkitid'];
					echo("\t\t\t\t<tr>\n\t\t\t\t\t<td>".$arr['indexkitname']."</td>\n\t\t\t\t\t<td>".$currentStatus."</td>\n");
					echo("\t\t\t\t\t<td><input class='checkbox' type='checkbox' name='".$arr['indexkitid']."' value='change'></td>\n");
					echo("\t\t\t\t</tr>\n");
				}
				$_SESSION['input'] = $indexkitArr;
				echo("\t\t\t</table>\n");
