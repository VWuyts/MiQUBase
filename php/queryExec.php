 ...
 */
	...
	if (isset($_SESSION['role']) && (strcmp($_SESSION['role'], 'executor') == 0 || strcmp($_SESSION['role'], 'creator') == 0))
	{
		...
		else
		{
			...
			$fields = array(
				...
				'run'				=> array('runID', 'runNumber', 'instrumentSN', 'startDate', 'libraryPrepKitID', 'loadingConcentration', 'kapaQuantification', 'indexKitID',
												'numCycles', 'sequencingCartridge', 'organismID', 'remark', 'pooledLibraryConcentration'),
			...
			if (isset($_SESSION['dbtable']))
			{
				...
				for ($i = 0; $i < count($_SESSION['dbtable']); $i++)
				{
					...
					for ($j = 0; $j < count($fields[strtolower($_SESSION['dbtable'][$i])]); $j++)
					{
						...
						if (strcmp($_SESSION['dbtable'][$i], 'run') == 0 && $j == 12)
						{
					...
					echo("\t\t\t\t<tr>\n\t\t\t\t\t<td colspan='8'><input name='". $_SESSION['dbtable'][$i] ."' class='checkbutton' type='button' value='Check all fields' />");
					echo("<input name='". $_SESSION['dbtable'][$i] ."' class='uncheckbutton' type='button' value='Uncheck all fields' /></td>\n\t\t\t\t</tr>\n\t\t\t</table>\n");
				}
				...
				for ($i = 0; $i < count($_SESSION['fields']); $i++)
				{
					for ($j = 0; $j < count($_SESSION['fields'][$_SESSION['dbtable'][$i]]); $j++)
					{
							...
							|| ($i == (count($_SESSION['fields']) - 1) && $j != (count($_SESSION['fields'][$_SESSION['dbtable'][$i]]) - 1)))
						{
							echo("\t\t\t\t<tr>\n\t\t\t\t\t<td colspan='7'></td>\n\t\t\t\t</tr>\n");
							echo("\t\t\t\t<tr>\n\t\t\t\t\t<td colspan='2'> </td>\n");
							echo("\t\t\t\t\t<td class='check'><input type='radio' name='andor".$i."_".$j."' value='AND' /></td><td>AND</td>\n");
							echo("\t\t\t\t\t<td class='check'><input type='radio' name='andor".$i."_".$j."' value='OR' /></td><td>OR</td>\n");
			...
			if (!empty($_SESSION['fields']))
			{
				...
				echo("\t\t\t\t\t<td class='check'></td><td class='dbitem'></td>\n\t\t\t\t\t<td class='check'></td><td class='dbitem'></td>\n\t\t\t\t\t<td class='check'></td>\n\t\t\t\t\t<td class='dbitem'></td>\n\t\t\t\t</tr>\n");
				...
				echo("\t\t\t\t\t\t<select class='orderby' name='orderby'>\n");
				for ($i = 0; $i < count($_SESSION['fields']); $i++)
				{
					for ($j = 0; $j < count($_SESSION['fields'][$_SESSION['dbtable'][$i]]); $j++)
					{
