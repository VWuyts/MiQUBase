 ...
 */
	...
	if (isset($_SESSION['role']) && isset($_SESSION['input']) && (strcmp($_SESSION['role'], 'administrator') == 0 || strcmp($_SESSION['role'], 'creator') == 0))
	{
		...
		{
			// Create array for updated libraryprepkitid
			$idArr = array();
			// Perform required updates
			$query = "UPDATE libraryprepkit
						SET isactive = $1
						WHERE libraryprepkitid = $2;";
			for ($i = 0; $i < count($_SESSION['input']['id']); $i++)
			{
				...
				if (isset($_POST[$_SESSION['input']['id'][$i]]))
				{
					...
					else
					{
						...
						// Create log message
						$activitylogger->info('status change of libraryprepkit succeeded', ['user'=>$_SESSION['user'], 'libraryprepkitid'=>$_SESSION['input']['id'][$i], 'changedto'=>$params[0]]);
					}
				}
			}
			unset($_SESSION['input']);
			if (count($idArr) == 0)
			{
				echo("\t\t<p class='message'>No libraryPrepKits have changed their status.</p>\n");
			}
			else
			{
				$query = "SELECT libraryprepkitname, isactive
							FROM libraryprepkit
							WHERE libraryprepkitid = ". $idArr[0];
				for ($i = 1; $i < count($idArr); $i++)
				{
					$query .= " OR libraryprepkitid = ".  $idArr[$i];
				}
				$query .= " ORDER BY libraryprepkitname ASC;";
				...
				{
					echo("\t\t<h1>LibraryPrepKits of which the status is changed</h1>\n");
					echo("\t\t<table>\n");
