 ...
 */
	...
	if (isset($_SESSION['role']) && (strcmp($_SESSION['role'], 'administrator') == 0 || strcmp($_SESSION['role'], 'creator') == 0))
	{
		...
		createHead(true, 'MiQUBase repeats', ['actions'], $script);
		...
		{
			// Add repeat=false and remarks to database
			if (isset($_POST['removeToRepeat']))
			{
				if (empty($_POST['sample']))
				{
					unset($_POST['run']);
					unset($_POST['laneid']);
					unset($_POST['removeToRepeat']);
					unset($_POST['sample']);
					unset($_POST['remark']);
					...
					createMessagePage(["No samples of run ". $_SESSION['runnumber'], "have been marked 'not to repeat' in MiQUBase."],
						$_SESSION['user'], "../php/administrativeTasks.php", "Back to tasks overview page");
					die();
				}
				else
				{
					...
					for ($i = 0; $i < $samplesToRepeat; $i++)
					{
						...
						{
							// Create log message
							$activitylogger->info('remove torepeat to db succeeded', ['user'=>$_SESSION['user'], 'runnumber'=>$_SESSION['runnumber'] , 'sampleid'=>$_POST['sample'][$i], 'remark'=>$newRemarkArr[$_POST['sample'][$i]]]);
						}	
					}			
				}
				unset($_POST['run']);
				...
				pg_close($dbconn);
				// Redirect user to message page
				createMessagePage([$samplesToRepeat ." samples of run ".$_SESSION['runnumber'], "have been marked 'not to repeat' in MiQUBase."],
					$_SESSION['user'], "../php/administrativeTasks.php", "Back tasks overview page");
				unset($_SESSION['runnumber']);
				die();
			}
			...
			if (isset($_POST['run']) && !empty($_POST['laneid']))
			{
				...
				$query = "SELECT sample.sampleid, sample.samplename, sample.remark,
							...
							WHERE sample.laneid=$1 AND sample.torepeat=TRUE;";
				...
				if (!$result)
				{
					...
					if (isset($_POST['removeToRepeat'])) unset($_POST['removeToRepeat']);
				...
				elseif (pg_num_rows($result) == 0)
				{
					...
					echo("\t\t<p class='message'>that can be marked not to be repeated.</p>\n");
			...
			$_SESSION['runnumber'] = '';
			$query = "SELECT DISTINCT run.runnumber, lane.laneid
						FROM run
							LEFT JOIN lane
			...
			else
			{
				...
				if (!isset($_POST['run']))
				{
					...
					echo("\t\t\t\t<a class='buttonMarginRight' href='administrativeTasks.php'>Cancel</a>\n");
					echo("\t\t\t</div>\n");
				}
				echo("\t\t</form>\n");
			}
			...
			if (isset($_POST['run']) && !empty($_POST['laneid']))
			{
				...
				echo("\t\t<h1 class='top'>Check which samples of run ". $_SESSION['runnumber'] ." not have to be repeated</h1>\n");
				echo("\t\t<form method='post' action=". htmlspecialchars($_SERVER['PHP_SELF']) .">\n");
				echo("\t\t\t<table>\n");
				for ($i = 0; $i < count($_SESSION['sampleidArr']); $i++)
				{
						...
						&& strcmp($currentSpecies, $speciesArr[$_SESSION['sampleidArr'][$i]]) == 0)
					{
						...
						echo("\t\t\t\t\t<td><input class='samples ".$class."' type='checkbox' name=sample[] value='". $_SESSION['sampleidArr'][$i] ."' />". $samplenameArr[$_SESSION['sampleidArr'][$i]] ."</td>\n");
						if ($counter % 2 == 0)
						{
							echo("\t\t\t\t</tr>\n");
						}
					...
					else
					{
						...
						if ($counter != 0)
						{
							...
							echo("\t\t\t\t<tr>\n\t\t\t\t\t<td colspan='4'><input id='".$class."' class='checkbutton' type='button' value='Check all samples' />");
							echo("<input id='".$class."' class='uncheckbutton' type='button' value='Uncheck all samples' /></td>\n\t\t\t\t</tr>\n");
						}
				...
				if ($counter != 0)
				{
					...
					echo("\t\t\t\t<tr>\n\t\t\t\t\t<td class='attribute'>Remarks:</td>\n");
					echo("\t\t\t\t\t<td colspan='2'><textarea name='remark[]'> </textarea></td>\n\t\t\t\t</tr>\n");
					echo("\t\t\t\t<tr>\n\t\t\t\t\t<td colspan='3'><input name='".$class."' class='checkbutton' type='button' value='Check all samples' />");
					echo("<input name='".$class."' class='uncheckbutton' type='button' value='Uncheck all samples' /></td>\n\t\t\t\t</tr>\n");
				}
				echo("\t\t\t</table>\n");
				...
				echo("\t\t\t\t<input class='button' type='submit' name='removeToRepeat' value='Add to MiQUBase' />\n");
				echo("\t\t\t</div>\n\t\t</form>\n");
			}
		}
